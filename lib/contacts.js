...
export let listAddressBooks = co.wrap(function *(account, options) {
  ...
  let addressBooks = responses
    .filter(res => {
      return true;
      // Don't filter iCloud addressbooks
 ...
 */
 ...
 /* forceDeleteCard does not pass an ETag header to the Dav server so the
  * card is always deleted, regardless if the card has been updated on the
  * server since we originally fetched it.
  */
export function forceDeleteCard(card, options) {
  return webdav.deleteObject(
    card.url,
    undefined,
    options
